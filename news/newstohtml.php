...
function news_to_html($file_name, $title, $rss_url=false, $more_url=false, $add_permalink=true, $format='long', $count=1000) {
	...
	foreach ($xml->item as $item) {
		...
		if ($format == 'long') {
			...
			$item_xml = $item->asXML();
			$item_xml = preg_replace('/<item.*?>/', '<p class="text-justify">', $item_xml);
			$item_xml = preg_replace('/<\/item>/', '</p>', $item_xml);
            $item_xml = preg_replace('/<p>/', '<p class="text-justify">', $item_xml);
            $item_xml = preg_replace('/<ul>/', '<ul class="text-justify">', $item_xml);
			$html .= $item_xml;
		}  
		/*$html .= "</td></tr>";*/
        $html .= "<hr class=\"custom-divider\">";
